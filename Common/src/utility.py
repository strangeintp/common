...
def popVariance(values):
    mu = mean(values)
    square_diffs = [(value-mu)**2 for value in values]
    return mean(square_diffs)

def sampleVariance(values):
    mu = mean(values)
    square_diffs = [(value-mu)**2 for value in values]
    return sum(square_diffs)/(len(values)-1)

def popStdDev(values):
    return math.sqrt(popVariance(values))

def sampleStdDev(values):
